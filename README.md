...
제한된 색상 수의 리퀴드 글래스 패널과 데스크톱의 스티키 탐색 축, 모바일의 접이식 메뉴를 중심으로 한 디자인을 유지하면서 다음 기능을 제공합니다.
...
- `unit-violet` — **자수정 관로**: 기본 남보라 바탕과 보라·민트 강조색
- `ember-white` — **잔불 벽돌**: 적갈색 바탕과 주황·적색 강조색
- `azure-form` — **청람 수문**: 청색 바탕과 하늘색·연보라 강조색
- `sovereign-obsidian` — **흑요 암거**: 흑요색 바탕과 금색·장미색 강조색
- `neon-shell` — **녹광 배수로**: 녹청색 바탕과 민트·주황 강조색
...
PC에서는 `검색` 버튼과 단축키 안내가 표시됩니다. 모바일에서는 공간을 차지하지 않도록 돋보기 버튼만 표시됩니다.
...
검색 결과는 제목 일치도를 가장 높게 평가하고, 그다음 연재명·주제·카테고리·설명·본문 일치도를 반영해 정렬합니다. 동점이면 작성 시각이 최근인 글을 우선합니다.
...
GoatCounter 사이트 코드와 요약 카운터 캐시 설정은 한 곳에서 관리합니다.
...
- 게시물 제목과 부제 아래에는 `게시 일자`, `예상 완독 시간`, `누적 방문`을 각각 배지로 표시합니다.
- 게시물 카운터 경로는 canonical URL을 우선 사용해 레거시 URL과 통계가 갈라질 가능성을 줄입니다.
...
본문의 일반 내용 뒤, 각주·연재·이전/다음 탐색과 작성 정보 앞에 세 개의 도구가 표시됩니다.
...
본문 폭과 글자 크기, 행간은 `styles/design-system.css`의 읽기 전용 토큰을 단일 기준으로 사용합니다.
...
- 데스크톱의 일반 본문은 최대 43rem(루트 16px 기준 688px) 안에 배치
- 이미지·표·코드는 필요할 때 본문보다 넓게 표시하거나 내부에서 독립적으로 가로 스크롤
- 560px 이하 포스트 화면은 바깥 여백 0.5rem과 카드 안쪽 여백 0.75rem을 합쳐 좌우 약 20px 확보
- 제목과 본문 소제목은 한국어 어절 중간 분리를 피하도록 `word-break: keep-all` 사용
- 비게시물 화면의 셸 여백과 정보 구조는 그대로 유지

관련 파일:

- `styles/design-system.css`
- `styles/reading-experience.css`
- `styles/index.css`
...
│  ├─ site-preferences.js          # 사이트·브라우저·giscus 테마와 저장값 동기화
│  ├─ post-actions.js              # 공유, 인쇄, 완독 시간, 각주, 목차, 인용 카드, giscus 지연 로딩
│  └─ refresh-reading-pulse.mjs    # 공개 GoatCounter 정적 스냅샷 생성
├─ styles/
│  ├─ design-system.css            # 색상 외 공통 디자인 토큰과 한국어 읽기 토큰
│  ├─ theme-system.css             # 다섯 색상 팔레트와 테마 선택기
│  ├─ index.css                    # 공통 컴포넌트와 반응형 레이아웃
│  ├─ home-editorial.css           # 홈 편집 화면
│  ├─ reading-experience.css       # 장문 본문 폭·타이포그래피·넓은 콘텐츠·인쇄
│  ├─ sidebar-collapse.css         # 모바일 접이식 메뉴
│  ├─ counter.css                  # 공개 카운터 컴포넌트 디자인
...
│  ├─ giscus-sovereign-obsidian.css # 흑요 암거 댓글 테마
