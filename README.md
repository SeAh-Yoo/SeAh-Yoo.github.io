...
> 게임을 비롯한 서브컬처, 영화, 인터넷, AI와 사회에서 발견한 질문과 잔여물이 흘러드는 이상 공간입니다.

**웹사이트:** https://seah-yoo.github.io/

## 소개

이 저장소는 GitHub Pages와 Jekyll로 운영되는 유세아의 개인 칼럼 블로그입니다.

「문하수도」는 가상의 기관이나 통신 서비스가 아니라, **유세아가 혼자 관리하는 지하의 이상 기록 공간**을 콘셉트로 삼습니다. 게임·서브컬처·영화·인터넷·AI와 사회에서 발견한 질문, 오래 생각한 것과 잊히기 아까운 문화의 잔여물이 이곳에 쌓입니다.

화면은 장문 비평의 가독성을 최우선으로 하며, 하수 관로·합류점·통로·흔적·수집물 같은 공간적 은유를 탐색 장치와 짧은 안내문에만 사용합니다. 본문을 관제 화면이나 설정집처럼 꾸미기보다, 읽기 편한 개인 에디토리얼 아카이브를 유지합니다.
...
- 장문 비평의 가독성을 세계관 연출보다 우선합니다.
- 본문 폭, 글자 크기, 행간과 정보 위계는 모든 색상 테마에서 동일합니다.
- 세계관은 메뉴의 보조 명칭, 탐색 구조, 짧은 안내문과 미세한 시각적 표식으로만 드러냅니다.
- 큰 메뉴명은 기능을 명확히 설명하고, 작은 보조 명칭은 공간 은유를 담당합니다.
- 통신망·공기업·특무기관식 용어보다 통로·흔적·잔여물·관리인 같은 개인 공간의 언어를 사용합니다.
- 도시괴담적 표현은 기능을 방해하지 않는 범위에서 제한적으로 사용합니다.

## 브랜드와 화면 문구 관리

브랜드명, 공간 관리인 정보, 홈 화면, 탐색 메뉴, 페이지 제목·소개, 검색 문구, 방문 현황 지표와 테마 표시명은 `_data/site_identity.yml`에서 관리합니다.

레이아웃과 페이지 파일에는 가능한 한 표시 문구를 직접 적지 않습니다. `_layouts/home.html`, `about.html`, `_layouts/page.html`, `_includes/sidebar.html`과 `_includes/head.html`은 중앙 설정의 값을 조합해 화면과 SEO 메타데이터를 생성합니다.
...
| `identity_key` | 경로 | 화면 기능 | 공간형 보조 명칭 |
| --- | --- | --- | --- |
| `home` | `/` | 홈 | 문하수도 |
...
사이드바의 `공간 설정`에서 다섯 가지 테마를 선택할 수 있습니다. 테마는 색상 분위기만 바꾸며 타이포그래피, 간격과 레이아웃은 바꾸지 않습니다.

- `unit-violet` — **오염된 관로**: 남보라 바탕과 보라·민트 강조색
...
선택값은 `localStorage`의 `literary-underground:preferences:v1`에 저장됩니다. `scripts/site-preferences.js`는 사이트 본문, 브라우저 테마 색상과 giscus 사용자 정의 테마를 함께 갱신합니다.

## 발견과 아카이브

사이드바의 `탐색기`와 `보조 탐색기`에서 아래 정적 페이지로 이동합니다. 목록은 Jekyll 빌드 시 게시물 Front Matter를 읽어 자동으로 생성됩니다.

| 주소 | 역할 | 작성자가 관리할 값 |
| --- | --- | --- |
| `/start-here/` | 처음 읽기 · 중심 합류점 | `start_here`, `start_here_order` |
| `/about/` | 공간 안내 · 이곳은 어디 | `_data/site_identity.yml`의 `operator`, `pages.about` |
...
이 사이트는 GoatCounter를 사용합니다. 표시되는 숫자는 개인을 식별한 독자 수의 확정값이 아니라 세션 기반의 익명 방문 흐름입니다.
...
│  ├─ search.css
│  └─ post-actions.css
├─ start-here.html                 # 처음 읽기 · 중심 합류점
├─ about.html                      # 공간 안내 · 이곳은 어디
├─ timeline.html                   # 전체 게시물 · 공간 이력
├─ reading-pulse.html              # 방문자 현황 · 남겨진 발자국
├─ topics.html                     # 소재별 분류 · 공간 계통
├─ references.html                 # 출처 및 인용 · 수집물 보관함
├─ categories.html                 # 주제별 분류 · 하수 관로
├─ search.json
├─ feed.xml
├─ index.html
└─ README.md
...
검색, 중심 합류점, 공간 계통, 공간 이력, 수집물 보관함, 남겨진 발자국, 예상 완독 시간, 각주와 참고 문헌, 인용 카드, 목차, 연재, 이전·다음 글, RSS와 인쇄 레이아웃은 별도의 유료 서버 없이 정적으로 동작합니다.
