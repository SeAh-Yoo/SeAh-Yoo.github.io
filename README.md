...
### RP 이름 후리가나와 같은 문서 제목 링크

위키 본문에서 RP 이름 위에 읽는 이름을 작게 표시할 수 있습니다. 공백 없는 직전 단어는 축약형을,
공백·문장부호·강조·링크가 포함된 본문은 중괄호 확장형을 사용합니다.
...
후리가나는 브라우저의 ruby 기준선에 맞춰 본문과 한 단위로 배치하고 실제 표시 너비가 비슷한 경우에만
짧은 행의 자간을 제한적으로 늘립니다. 한 글자와 너비 차이가 큰 표기는 가운데 정렬하며,
화면보다 긴 표기는 모바일에서 내부 줄바꿈합니다.
후리가나 안의 기존 Markdown 링크와 플랫폼 선언은 같은 URL·아이콘·새 탭 정책을 유지합니다.

현재 문서의 제목에는 다음처럼 표시명과 제목 표시 텍스트를 적어 연결합니다. 링크보다 뒤에 있는 제목과
강조·일반 링크가 포함된 제목도 찾으며 Kramdown이 생성한 실제 제목 ID를 그대로 사용합니다.
...
같은 제목이 여러 개면 첫 번째로 연결하고 진단 상태를 남깁니다. 없는 제목은 가짜 링크를 만들지 않고
표시명을 평문으로 보존합니다. 코드 블록·인라인 코드·이스케이프된 표기에는 적용하지 않으며, 빈 값이나
닫히지 않은 괄호도 원문을 보존합니다. 본문 범위, 파싱 우선순위, 예외와 후속 편집 기능 제안은
[`docs/wiki-markdown.md`](docs/wiki-markdown.md)에 정리되어 있습니다.
...
위키에만 `##`부터 `########`까지 `1. → 가. → 1) → 가) → [1] → [가] → a.` 번호를 붙입니다.
부모가 바뀌면 하위 번호가 다시 시작합니다. 문서 제목과 자동 생성되는 요약·상세 페이지·역링크는 제외합니다.
한글은 `하 → 가가 → 가나 → … → 가하 → 나가 → … → 나하 → … → 하하 → 가가가`로,
영문은 `z → aa → … → zz → aaa`로 이어집니다.
일반 포스트의 제목과 목차는 그대로 유지합니다. 제목 단계를 건너뛰면 빌드 경고를 표시합니다.

```markdown
## 기본적으로 접힌 구역 [_접기] {#stable-section}
...
여기는 앞 구역의 접기에 포함되지 않습니다.
```

제목 끝에 공백과 `[_접기]`를 붙입니다. 명시적 `{#id}`는 그 뒤에 둡니다.
제목은 남기고 다음 동급·상위 제목 직전까지 접습니다. 틀·목록·기존 `<details>` 안의 제목은
해당 컨테이너의 끝에서도 접기 범위가 끝나므로 바깥 구조를 이동시키지 않습니다.
책 SVG와 열기/접기 버튼으로 조작하며, 부모를 다시 열어도 하위 구역 상태는 유지합니다.
새로 방문하면 기본 접힘 상태로 돌아갑니다. JavaScript가 꺼져 있으면 본문 전체가 보입니다.

위키 본문 글자 표기 영역은 일반 포스트보다 약 10~25% 넓으며 화면이 작으면 화면 폭에 맞춥니다.
3~7단계 제목은 2단계와 같은 크기로 표시하고 3단계부터 단계별로 들여씁니다. 1~2단계는 들여쓰지 않습니다.
위키 목록과 문서 상단 목차는 `가.`, `1)`처럼 2단계부터 2~3글자씩 순차적으로 들여쓰며,
각 제목 아래의 본문도 해당 제목보다 2~3글자 안쪽에 배치합니다.
기존 `<details markdown="1"><summary>제목</summary>…</details>` 문법과 `open` 속성은 유지하며,
위키에서만 제목과 책 아이콘·열기/접기 표시를 새 접기와 통일합니다. 기존 접기는 JavaScript 없이도 동작합니다.
역링크 목록과 댓글은 문서 본문 패널 밖의 별도 패널에 표시하며, 댓글 패널은 역링크 목록 아래에 배치합니다.

번호와 접기 표기는 제목 ID에 영향을 주지 않습니다. 위키 목록과 상단 목차에서는 번호·제목 전체가 링크이며,
본문 제목은 일반 글자입니다. 접힌 대상의 링크로 이동하면 대상 및 상위 접기를 자동으로 엽니다.
인쇄할 때는 펼치고 이후 원래 상태로 복원합니다. Chrome의 브라우저 찾기는 숨겨진 내용도 펼쳐 찾을 수 있습니다.
코드·본문 안의 `[_접기]`는 그대로 표시합니다. 제목에 표기를 문자로 쓰려면 `\[_접기]`로 이스케이프합니다.

검증 명령은 `ruby scripts/test-wiki-sections.rb`와 `python scripts/test-wiki-sections.py`입니다.
